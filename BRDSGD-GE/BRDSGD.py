##The code for this project is partially derived from  https://github.com/Zhaoxian-Wu/IOS
...
from ByrdLab.library.learnRateController import ladder_lr, one_over_sqrt_k_lr, one_over_k_lr
...
parser.add_argument('--aggregation', type=str, default='scc')
parser.add_argument('--attack', type=str, default='isolation')
parser.add_argument('--data-partition', type=str, default='noniid')
parser.add_argument('--lr-ctrl', type=str, default='1/k')
...
if args.graph == 'CompleteGraph':
    ...
    graph = TwoCastle(k=6, byzantine_size=2, seed=14)
elif args.graph == 'ER':
    honest_size = 20
    byzantine_size = 4
...
elif args.lr_ctrl =='1/k':
    lr_ctrl = one_over_k_lr(a=1, b=1)
...
elif args.aggregation == 'ios_B':
    aggregation = D_ios(graph, exact_byz_cnt=False, byz_cnt=4)
elif args.aggregation == 'ios_equal_neigbor_weight':
    aggregation = D_ios_equal_neigbor_weight(graph)
elif args.aggregation == 'trimmed-mean_B':
    aggregation = D_trimmed_mean(graph, exact_byz_cnt=False, byz_cnt=4)
...
log('[Start Running]')
_, loss_path, acc_path, consensus_error_path, diff_path, train_loss_path = env.run()
