...
from ByrdLab.library.measurements import avg_loss_accuracy_dist, consensus_error, avg_loss_accuracy_dec
...
class DSGD(Dec_Byz_Opt_Env):
    ...
    def run(self):
        ...
        hint = '[DSGD]' + num_format + '/{} iterations ({:>6.2f}%) ' + \
            'test_loss={:.3e}, accuracy={:.4f}, ce={:.5e}, diff={:.3e},train_loss={:.3e} , lr=={:.4f}'
        ...
        for iteration in range(0, self.total_iterations + 1):
            ...
            if iteration % self.display_interval == 0:
                ...
                train_loss_path.append(val_loss)
                log(hint.format(
                    iteration, self.total_iterations,
                    iteration / self.total_iterations * 100,
                    test_loss, test_accuracy, ce,  diff, val_loss, lr
                ))
                # reset the record
                # train_loss_avg = 0
            ...
            for node in self.graph.honest_nodes:
                ...
                for param in model.parameters():
                    penalization += self.weight_decay * param.norm() ** 2 / 2
                loss = loss + penalization
                model.zero_grad()
                loss.backward()
        ...
        return avg_model, loss_path, acc_path, consensus_error_path, diff_path, train_loss_path
...
class SimiliarityReweighting(Dec_Byz_Opt_Env):
    ...
    def run(self):
        ...
        hint = '[SimReweight]' + num_format + '/{} iterations ({:>6.2f}%) ' + \
            'loss={:.3e}, accuracy={:.4f}, ce={:.5e}, diff={:.3e},train_loss={:.3e},lr={:.4f}'
